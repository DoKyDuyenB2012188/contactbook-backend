...
exports.findAll = async (req, res, next) => {
    let documents = [];
    try {
        const contactService = new ContactService(MongoDB.client);//client
        console.log(contactService);
        const {name} = req.query;
        console.log(name);
        if(name){
            documents = await contactService.findByName(name).toArray(); //client.db().collection("contacts");
        }else{
...
exports.findAllFavorite = async (req, res, next) => {
    try {
        ...
        console.log(error);
        next(new ApiError(500,"An error occurred while retrieving favorite contacts"));
    }
}
